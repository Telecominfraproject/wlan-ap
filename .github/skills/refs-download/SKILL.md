...
```
/refs-download              # Default: clone all, openwifi-reference-docs md/ only
...
Most sources are cloned normally. The exception is **openwifi-reference-docs**,
which uses sparse checkout to skip the large `origin/` directory (PDFs):

```bash
# openwifi-reference-docs: sparse checkout — md/ only (indexes + markdown)
git clone --filter=blob:none --sparse <uri> refs/openwifi-reference-docs
cd refs/openwifi-reference-docs
...
```bash
# openwifi-reference-docs: full clone — everything
git clone <uri> refs/openwifi-reference-docs
```
...
```
/refs-download openwifi-reference-docs          # md/ only
/refs-download openwifi-reference-docs --full   # full clone
...
/refs-download update openwifi-reference-docs   # single source
```
...
2. **For each source**, determine clone strategy:
   - `openwifi-reference-docs` (default mode): sparse checkout `md/` only
   - `openwifi-reference-docs` (full mode): full clone
...
```bash
./scripts/refs-setup.sh                    # Clone all (default sparse for openwifi-reference-docs)
...
| openwifi-reference-docs | **~5-20 MB** (md/ only) | **~200+ MB** (with all PDFs) |
