...
network={
        ...
        password="$password"
        phase2="auth=$radiusauthtype"

}
EOF

result=$(eapol_test -c /tmp/Auth.conf -a "$radiusserverip" -p "$radiusserverport" -s "$radiusserversecret" | grep -c "eapol_sm_cb: result=1")
...
if [ ! -z "$status_present" ]; then
	...
	hid="0"
	gatewayaddress="0"
	queryvarlist="clientip gatewayname splashpagetitle acceptancepolicy loginsuccesstext redirecturl radiusserverip radiusserverport radiusserversecret radiusauthtype redir username password"
else
	queryvarlist="clientip gatewayname hid splashpagetitle acceptancepolicy loginsuccesstext redirecturl radiusserverip radiusserverport radiusserversecret radiusauthtype  redir gatewayaddress username password"
fi
...
radiusserverip=$(printf "${radiusserverip//%/\\x}")
htmlentityencode "$radiusserverip"
radiusserveriphtml=$entityencoded

radiusserverport=$(printf "${radiusserverport//%/\\x}")
htmlentityencode "$radiusserverport"
radiusserverporthtml=$entityencoded

radiusserversecret=$(printf "${radiusserversecret//%/\\x}")
htmlentityencode "$radiusserversecret"
radiusserversecrethtml=$entityencoded

radiusauthtype=$(printf "${radiusauthtype//%/\\x}")
htmlentityencode "$radiusauthtype"
radiusauthtypehtml=$entityencoded
...
login_form="
	...
	<input type=\"hidden\" name=\"radiusserverip\" value=\"$radiusserveriphtml\">
	<input type=\"hidden\" name=\"radiusserverport\" value=\"$radiusserverporthtml\">
	<input type=\"hidden\" name=\"radiusserversecret\" value=\"$radiusserversecrethtml\">
	<input type=\"hidden\" name=\"radiusauthtype\" value=\"$radiusauthtypehtml\">
