...
void a2w_logfr_and_reboot( char * Buf )
{
	char buf[200];
	memset(buf, '\0', sizeof(buf));
	snprintf(buf, sizeof(buf)-1, "websocket requested reboot (%s)", Buf);
	wc_put_logline( (WRITE_TO_FR | APP_LOG_WEBSOCK | DEBUG_LEVEL_1), "%s", buf );
	sleep(1);
	system( "/splashbin/reboot" );
}
...
#else
    wc_put_logline( (APP_LOG_WEBSOCK | DEBUG_LEVEL_1), "%s%s", buf, line);
#endif
#endif
}
