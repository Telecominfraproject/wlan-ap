...
function wifi_scan() {
	...
	for (let phy in phys) {
		...
		if (!frequency_list_5g[ch_width]) {
			warn("channel_width " + ch_width + " not found in frequency_list_5g, fallback to 3\n");
			ch_width = 3;
		}
		...
		let useVendorWhiteList = false;
		if (length(rogue_ap_cfg.rules) > 0 && rogue_ap_cfg.rules[0].vendor !== undefined)
			useVendorWhiteList = true;
