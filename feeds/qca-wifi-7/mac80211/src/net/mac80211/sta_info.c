...
{
	u32 rate =
		READ_ONCE(sta_get_last_rx_stats(sta, true, link_id)->last_rate);
			       ...
			       bool tidstats)
{
	...
	last_rxstats = sta_get_last_rx_stats(sta, false, link_id);
...
unsigned long ieee80211_sta_last_active(struct sta_info *sta, int link_id)
{
	...
	stats = sta_get_last_rx_stats(sta, false, link_id);
