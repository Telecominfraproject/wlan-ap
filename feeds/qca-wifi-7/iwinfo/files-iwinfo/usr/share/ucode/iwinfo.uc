...
export function assoclist(dev) {
	...
	for (let station in stations) {
		/*
		 * The kernel does not always report a full sta_info block, e.g. for a
		 * station that just associated and has not exchanged any data yet.
		 * Access the nested attributes defensively so that a single incomplete
		 * station does not abort the whole dump with a reference error.
		 */
