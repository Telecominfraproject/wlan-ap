...
static int qdss_create_buf_tbl(struct qdss_bridge_drvdata *drvdata)
{
	...
	drvdata->nr_trbs = mhi_get_free_desc_count(mhi_dev, DMA_FROM_DEVICE);
...
struct qdss_buf_tbl_lst *qdss_get_buf_tbl_entry(struct qdss_bridge_drvdata *drvdata, void *buf)
{
	struct qdss_buf_tbl_lst *entry;
...
static void qdss_buf_tbl_remove(struct qdss_bridge_drvdata *drvdata, void *buf)
{
	struct qdss_buf_tbl_lst *entry = NULL;
...
static void mhi_read_work_fn(struct work_struct *work)
{
	...
	struct qdss_bridge_drvdata *drvdata = container_of(work, struct qdss_bridge_drvdata,
							   read_work);
		...
		struct mhi_result *result)
{
	...
	drvdata = dev_get_drvdata(&mhi_dev->dev);
	if (!drvdata)
		return;
	buf = result->buf_addr;
...
static int qdss_mhi_probe(struct mhi_device *mhi_dev, const struct mhi_device_id *id)
{
	int ret;
	...
	drvdata->dev = device_create(mhi_class, NULL, drvdata->cdev->dev,
				     drvdata, "mhi_qdss");
