...
int main(int argc, char *argv[])
{
    ...
    static struct option options[] =
    {
        ...
        {"semaphore-timeout",required_argument,NULL,'s'},
    ...
    while (1)
    {
        c = getopt_long(argc, argv, "hdis:npb:ftc:", options, NULL);
        if (c < 0)
            break;
...
#endif
        case 'c':
            fp = fopen("/tmp/ftm.conf", "r");
            if (!fp) {
                DPRINTF(FTM_DBG_ERROR, "ftm.conf open failed.\n");
                exit(EXIT_FAILURE);
            }
            fscanf(fp, "total_num_slots=%d\n", &ftm_cfg.total_num_slots);
            for (i = 0; i < ftm_cfg.total_num_slots; i++) {
                fscanf(fp, "slot_%u_size=%uk\n", &ftm_cfg.slot_id[i], &ftm_cfg.slot_size[i]);
            }
            fclose(fp);
            break;
